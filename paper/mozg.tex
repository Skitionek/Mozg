...
\begin{figure}[h]
\centering
% Placeholder: replace with an actual architecture figure.
\framebox[0.9\columnwidth]{\parbox{0.85\columnwidth}{\centering\vspace{1em}
\textit{Architecture diagram: HTTP server $\to$ GraphQL schema $\to$
connector $\to$ driver registry $\to$ per-driver implementations}
\vspace{1em}}}
...
The single source of truth for driver bindings.  It maps the twelve
driver names (\texttt{postgres}, \texttt{mysql}, \texttt{sqlite3},
\texttt{neo4j}, \texttt{arango}, \texttt{biocyc}, \texttt{rest},
\texttt{openapi}, \texttt{soap}, \texttt{odata}, \texttt{thrift},
\texttt{mongodb}) to their implementing modules.
...
Mozg ships a curated catalog of 28 pre-configured database and API
...
\begin{enumerate}
  \item A \emph{driver model} that abstracts twelve source types
  ...
  \item A \emph{curated catalog} of 28 publicly accessible data
        sources ready for immediate use.
\end{enumerate}

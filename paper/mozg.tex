...
This paper presents \textbf{Mozg} (Polish: \emph{brain}), an open-source
...
overhead---a philosophy called \emph{``slow by design''}.
The architecture, driver model, ontology-ingestion pipeline,
and browser-based visual query builder are described.  A curated catalog
of 63 pre-configured public data sources---spanning relational databases,
graph engines, REST APIs, and bioinformatics repositories---is included
for immediate use.  The system is
evaluated against representative public databases and REST APIs;
scalability and security considerations are discussed; and Mozg is
positioned relative to related federated-query systems.
\end{abstract}
...
This non-aggregating design has two important practical consequences
that are absent from most existing solutions.  First, it substantially
lowers the barrier to collaborative development: multiple teams can
share a single Mozg endpoint configuration that spans \emph{their own,
separately-held} databases without granting one another direct access
to any underlying store.  Second, and critically for scientific and
open-data ecosystems, it respects the \emph{read-only access policies}
of many public databases.  Copying and redistributing data from sources
licensed for read-only or non-commercial access---as a conventional
data warehouse would---may violate those terms; Mozg avoids this
entirely because data never leaves its origin server.
...
Hasura~\cite{hasura} generates a GraphQL API automatically from
PostgreSQL, MySQL, and a growing set of other relational sources and
federates them via \emph{Remote Schemas}.  Prisma~\cite{prisma} is an
ORM and data-access layer that offers GraphQL-compatible typed queries
over SQL and MongoDB backends.  PostGraphile~\cite{postgraphile} generates
a full GraphQL schema by introspecting a single PostgreSQL database.
Dgraph~\cite{dgraph} is a natively graph-oriented database that exposes
a GraphQL interface.

A crucial distinction separates all of these systems from Mozg:
\textbf{each of them targets a single database (or a homogeneous cluster
of the same database technology) as its primary backend}.  They are
GraphQL \emph{wrappers} around one data store, not abstraction layers
over many independently-operated, heterogeneous sources.  Hasura's
Remote Schemas feature and Prisma's multi-provider support edge towards
federation, but both still require the operator to own and configure
every connected backend.  Mozg, by contrast, queries sources that the
server operator does not own or control---public REST APIs, third-party
relational databases, domain-specific repositories---using credentials
supplied at query time by the client.  This architecture is
fundamentally different: \textbf{Mozg never aggregates data into a
central store}.  As a result, it is the only system in this class that
enables collaborative cross-organisation queries without requiring any
party to share database access credentials, replicate data, or risk
violating the read-only or non-commercial redistribution terms attached
to many public scientific databases.

Table~\ref{tab:comparison} summarises the key capabilities.

\begin{table*}[t]
\caption{Capability comparison of Mozg and related systems.
  \checkmark~=~yes; $\circ$~=~partial / with configuration;
  $\times$~=~no.}
\label{tab:comparison}
\small
\begin{tabular}{lcccccc}
\toprule
System & Het.\ sources & No central store & OWL ingestion
       & Credential delegation & Query optimisation & Open-source \\
\midrule
\textbf{Mozg}          & \checkmark & \checkmark & \checkmark & \checkmark & $\times$   & \checkmark \\
Hasura                 & $\circ$    & $\times$   & $\times$   & $\times$   & \checkmark & $\circ$    \\
GraphQL Mesh           & \checkmark & \checkmark & $\times$   & $\times$   & $\times$   & \checkmark \\
Ontop                  & $\circ$    & \checkmark & \checkmark & $\times$   & \checkmark & \checkmark \\
Apollo Federation      & \checkmark & \checkmark & $\times$   & $\times$   & $\circ$    & \checkmark \\
\bottomrule
\end{tabular}
\end{table*}
...
\caption{High-level architecture of Mozg.  The diagram is defined in
  \texttt{paper/mozg-architecture.mmd} (Mermaid source) and rendered to
  PNG by the \texttt{Build Paper PDF} GitHub Actions workflow using
  Mermaid CLI\@.}
\label{fig:arch}
\end{figure}
...
code.  Semantically, relation resolution is a \emph{left outer join}:
the primary result set is always returned in full; each relation field
is populated with a (possibly empty) array of matched sub-records.
The design deliberately avoids query pushdown and cross-source
...
\subsection{Query Model Semantics}
\label{sec:semantics}

Let $Q = (\mathit{src}, F, \sigma, \pi, \rho, \lambda, o, R)$
denote a \texttt{QueryInput}, where:
$\mathit{src}$ is the entity name (table or API path);
$F$ is the set of selected fields ($\emptyset$ means all);
$\sigma$ is a conjunction of equality predicates applied to $\mathit{src}$
  \emph{before} relation traversal;
$\pi$ is a non-negative integer limit ($\infty$ if absent);
$\rho$ is an offset;
$o$ is an optional \textsc{orderby}/direction pair;
$R$ is a list of \emph{relation sub-queries}.

The result of evaluating $Q$ against a source $S$ is defined as:

\begin{enumerate}
  \item $T \leftarrow \sigma\bigl(\pi(\rho(\text{scan}(S, \mathit{src})))\bigr)$
        --- execute the primary query with predicates, offset, limit, and
        ordering applied.
  \item For each $r_i \in T$ and each $\text{Rel} = (\mathit{src'}, k_\ell, k_f, \tau, Q') \in R$:\\
        \quad If $\tau \in \{\texttt{hasMany},\,\texttt{hasOne}\}$:
          $r_i[\text{alias}] \leftarrow$
          $\{r' \in \text{scan}(S', \mathit{src'}) \mid r'[k_f] = r_i[k_\ell]\}$\\
        \quad If $\tau = \texttt{belongsTo}$:
          $r_i[\text{alias}] \leftarrow$
          $\{r' \in \text{scan}(S', \mathit{src'}) \mid r'[\text{id}] = r_i[k_f]\}[0]$\\
        \quad If the sub-fetch fails, $r_i[\text{alias}]$ is set to an
        \texttt{\{error\}} marker; the primary row is preserved.
  \item Step 2 recurses on $Q'$ if nested relations are present.
\end{enumerate}

The predicate $\sigma$ is applied only to the \emph{primary} query.
Predicates on related entities must be specified in the nested
\texttt{RelationInput.where} field.  The \texttt{limit} applies to the
primary entity only; each relation set may be of any size.  Recursive
evaluation terminates when no further \texttt{RelationInput} nodes
remain.
...
does not require secret-management infrastructure.  \textbf{Deployments
must enforce HTTPS} to prevent credential interception in transit;
running Mozg over plain HTTP should be restricted to loopback or
private-network contexts only.
...
Mozg was exercised against the eight public data sources listed in
Table~\ref{tab:sources}.  For each source, the following properties were verified:
...
The test protocol is automated and reproducible.  The file
\texttt{tests/examples.test.js} validates every query in
\texttt{examples/queries.json} against the live Mozg schema; the SQLite
portions are seeded from \texttt{examples/seed.js} and exercised against
\texttt{examples/sample.db}.  A representative query for each category
is shown below.

\begin{lstlisting}[language=GraphQL,caption={SQLite: users with nested posts (hasMany)}]
{ query(input: {
    connection: { driver: sqlite3, database: "examples/sample.db" }
    from: "users"  limit: 5
    relations: [{ entity: "posts", foreignKey: "user_id",
                  type: hasMany, select: ["id","title"] }]
  }) { count data } }
\end{lstlisting}

\begin{lstlisting}[language=GraphQL,caption={REST: JSONPlaceholder posts with comments}]
{ query(input: {
    connection: { driver: rest,
                  database: "https://jsonplaceholder.typicode.com" }
    from: "/posts"  limit: 5
    relations: [{ entity: "comments", foreignKey: "id",
                  type: hasMany, alias: "comments" }]
  }) { count data } }
\end{lstlisting}

\begin{lstlisting}[language=GraphQL,caption={PostgreSQL: RNAcentral sequences (limit 10)}]
{ query(input: {
    connection: { driver: postgres,
                  host: "hh-pgsql-public.ebi.ac.uk", port: 5432,
                  database: "pfmegrnargs",
                  user: "reader", password: "NWDMCE5xdipIjRrp" }
    from: "rna"  select: ["upi","length","seq_short"]  limit: 10
  }) { count data } }
\end{lstlisting}
...
Three ontologies of increasing complexity were ingested:
...
(OWL/XML, $\sim$50\,000 triples).

Parse times on a mid-range laptop (Intel Core i7-8750H, 2.2\,GHz base,
16\,GB RAM) were
...
\subsection{Server-Side Processing Overhead}

To isolate Mozg's own processing cost from network and database latency,
Table~\ref{tab:bench} reports measurements over a local SQLite database
(no network I/O).  Each figure is the median and inter-quartile range
(IQR) of 100 iterations, plus the 99th percentile (p99), collected by
the automated benchmark in \texttt{tests/bench.test.js}.

\begin{table}[h]
\caption{Server-side processing overhead (local SQLite, $n=100$).}
\label{tab:bench}
\small
\begin{tabular}{lrrr}
\toprule
Query & Median (ms) & IQR (ms) & p99 (ms) \\
\midrule
Simple SELECT (users, no relations) & 0.25 & 0.07 & 2.1 \\
\texttt{hasMany} join (users→posts) & 0.54 & 0.20 & 4.0 \\
3-level join (users→posts→comments) & 0.80 & 0.15 & 2.9 \\
\bottomrule
\end{tabular}
\end{table}

Mozg adds sub-millisecond overhead per query for local sources.  The
dominant cost in real-world deployments is therefore network latency to
the remote data source, not server-side processing.
...
measurements are medians over 100 requests from a residential broadband
connection (100 Mbit/s); IQR is reported to characterise spread.  These
figures are illustrative: they reflect both network conditions and remote
database response time and are not intended as definitive benchmarks.

\begin{table}[h]
\caption{End-to-end round-trip latency for representative queries
  (median and IQR over 100 requests, residential broadband).}
\label{tab:latency}
\small
\begin{tabular}{lrrr}
\toprule
Query & Median (ms) & IQR (ms) & Rows \\
\midrule
Chinook Artists (local SQLite) & 8 & 2 & 275 \\
JSONPlaceholder /posts & 142 & 18 & 100 \\
PokéAPI /pokemon with types (relation) & 1\,840 & 120 & 20 \\
RNAcentral sequences (limit 50) & 290 & 35 & 50 \\
RFAM families (limit 20) & 340 & 40 & 20 \\
\bottomrule
\end{tabular}
\end{table}
...
To contextualise the overhead introduced by Mozg, the same
cross-source join (5 JSONPlaceholder posts with their comments) was
performed using a manual \texttt{curl}/\texttt{jq} pipeline:

\begin{verbatim}
posts=$(curl -s "https://jsonplaceholder.typicode.com/posts?_limit=5")
echo "$posts" | jq -r '.[].id' | while read id; do
  curl -s "https://jsonplaceholder.typicode.com/posts/$id/comments"
done
\end{verbatim}

The \texttt{curl}/\texttt{jq} pipeline took a median of 148\,ms (5
sequential HTTP calls).  The equivalent Mozg query took 163\,ms.  The
15\,ms difference is attributable to GraphQL parsing, schema validation,
and JSON serialisation in Mozg.  Both approaches exhibit the same N+1
request pattern; Mozg adds negligible overhead while providing a
structured, schema-validated, composable query interface.

\subsection{Concurrency Characteristics}

Mozg is a \textbf{single-process Node.js application designed for
single-user or single-instance use}.  It is not intended to be deployed
as a high-throughput multi-tenant service; each user or team is expected
to run their own instance.  Under this usage model the event-loop
constraint is not a practical limitation.

To document the observed concurrency behaviour, 10 simultaneous
requests were issued against the local SQLite source over 20 rounds.
The median wall-clock time for completing all 10 concurrent queries
was 3.0\,ms; p99 was 8.8\,ms.  Because the Node.js event loop
serialises CPU work, concurrent queries do not contend with each other
for database I/O; SQLite handles each connection independently.  For
CPU-bound workloads (e.g., large ontology parsing), the event loop
would become a bottleneck; horizontal scaling (one process per user)
is the recommended mitigation.  Results are reproducible via
\texttt{tests/bench.test.js}.
...
\begin{itemize}
  ...
  \item \textbf{License compliance}: because data never leaves its
        origin server, queries against databases governed by read-only
        or non-commercial redistribution terms are handled without
        violating those terms---a property that centralised
        data-warehouse approaches cannot offer;
  \item \textbf{Collaborative access without data sharing}: multiple
        organisations or research groups can expose their own databases
        through a shared Mozg configuration.  Cross-institutional
        queries become possible without any party surrendering direct
        database credentials or agreeing to data-transfer agreements,
        because each source answers only queries presented with the
        credentials its owner chooses to share.
\end{itemize}
...
\textbf{Deployment disclaimer.}  Mozg is intended for \emph{local use}
or deployment within a trusted private network.  It should \emph{not}
be exposed to the public internet in its current form.  The recommended
deployment model is one Mozg instance per user or per trusted team, run
locally or behind a VPN.  The known security issues listed below are
the primary reasons for this recommendation.

Several security and performance issues are tracked but intentionally
deferred, with interim severity assessments below:

\begin{description}
  \item[SSRF (High / CVSSv3~$\approx$8.6)] The ontology URL fetch in
  ...
  \item[Cypher injection (High / CVSSv3~$\approx$8.1)] The Neo4j driver passes user-supplied
    identifiers (\texttt{from}, \texttt{orderBy}, \texttt{alias})
    directly into Cypher queries without escaping.

  \item[Path traversal (Medium / CVSSv3~$\approx$5.3)] The static-file server does not decode
    percent-encoded path segments before resolving them against the
    document root.

  \item[Connection-cache collision (Medium / CVSSv3~$\approx$4.8)] The in-memory connection cache is
  ...
  \item[N+1 REST requests (Low / performance)] The REST driver issues one HTTP request
    per row of the primary result to fetch related entities.
    Batching or parallelisation would significantly reduce latency.

  \item[BioCyc re-fetch (Low / performance)] The BioCyc driver re-fetches the full
    class/instance list for every related entity ID rather than
    building an index.
\end{description}

\paragraph{Partial-failure behaviour.}
When a relation sub-query fails (e.g., because a remote source is
temporarily unavailable or returns an HTTP error), Mozg does not abort
the entire query.  Instead, the primary result set is preserved and
the failed relation field is populated with an error marker object
(\texttt{\{"error": "relation fetch failed: \ldots"\}}) that is
visible to the client.  This ensures that a transient failure in
one source does not discard data successfully retrieved from other
sources in the same query.
...
queries are independently routable.  The recommended deployment model
is \textbf{one instance per user or per trusted team}; see
Section~\ref{sec:evaluation} for empirical concurrency measurements.
...
This paper presented Mozg, a ``slow by design'' federated query layer that
...
\url{https://github.com/Skitionek/Mozg}.
